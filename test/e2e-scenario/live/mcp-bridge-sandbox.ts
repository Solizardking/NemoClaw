...
export async function setupDnsRebindingHostsFixture(
  ...
  const fixture = {
    hostname,
    hostBackupPath: path.join(tempDir, `nemoclaw-mcp-rebind-hosts-host-${suffix}`),
    sandboxBackupPath: path.join(tempDir, `nemoclaw-mcp-rebind-hosts-sandbox-${suffix}`),
  };
  const result = await host.command(
    ...
    {
      artifactName: "mcp-dns-rebinding-backup-hosts",
  ...
  const result = await host.command(
    ...
    [
      ...
      [
        ...
        'if [ -f "$host_backup" ]; then sudo -n tee /etc/hosts < "$host_backup" >/dev/null; fi',
        'container_id="$(docker ps --filter "label=openshell.ai/sandbox-name=${sandbox_name}" --format \'{{.ID}}\' 2>/dev/null | head -n 1 || true)"',
        'if [ -n "$container_id" ] && [ -f "$sandbox_backup" ]; then docker exec --user 0 -i "$container_id" sh -c \'cat > /etc/hosts\' < "$sandbox_backup"; fi',
        'rm -f "$host_backup" "$sandbox_backup"',
      ].join("\n"),
    ],
    {
      artifactName: "mcp-dns-rebinding-restore-hosts",
      env: buildAvailabilityProbeEnv(),
      timeoutMs: 60_000,
    },
  );
