...
# ABOUTME: Handles prompt caching, summarization, context window management, and skill registry integration
...
class ContextManager:
    ...
    Integrates with the SkillRegistry to inject available skill documentation
    into agent prompts.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            token_address: Solana mint address injected as context header
            skills_dir: Path to skills/ directory for automatic skill injection
            max_skills_detail: Number of skills to show full content for
                              (0 = compact index only, -1 = show all)
        """
        ...
        self.max_skills_detail = max_skills_detail
    ...
    def skill_registry(self):
        """Lazy access to the SkillRegistry instance."""
        if self._skill_registry is None and self.skills_dir is not None:
    ...
    def get_prompt(self) -> str:
        """Get the current prompt with optimizations and skill injection."""
        # Use direct prompt text if provided
        if self.prompt_text:
        ...
        # Prepend token context if set
        if self.token_address:
            token_header = f"## Target Token\nSolana mint address: `{self.token_address}`\n\n"
            base_content = token_header + base_content

        # Append skills section if registry is loaded
        skills_section = self._get_skills_section()
        if skills_section:
            if len(base_content) + len(skills_section) < self.max_context_size:
                base_content += "\n\n" + skills_section
            else:
                # Too large for full section — use compact index only
                if self.skill_registry:
                    compact = self.skill_registry.compact_index()
                    base_content += f"\n\n## Available Skills\n\n{compact}\n\n*{self.skill_registry.count} skills available. Ask for details on any skill.*"
    ...
    def _get_skills_section(self) -> str:
        ...
        Uses ``format_prompt_section`` from the SkillRegistry with
        the configured ``max_skills_detail``.
        ...
        if self.max_skills_detail < 0:
            # Show all skills in full
            return registry.full_catalog()

        return registry.format_prompt_section(
