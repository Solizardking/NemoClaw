...
### Jupiter Ultra Tools (4 tools) — USE THESE for one-shot buys of any token:
30. **jupiter_buy_token** - ⚠️ Buy ANY token with SOL (memecoins, pump.fun, etc.) — one-shot
31. **jupiter_sell_token** - ⚠️ Sell ANY token for SOL — one-shot
32. **jupiter_swap** - ⚠️ Swap any token for any other token — one-shot
33. **jupiter_get_quote** - Preview a Jupiter swap (price impact, expected output)

### Wallet Activity Tools (1 tool):
34. **get_wallet_transfers** - Get token transfer history (sent/received) via Helius
...
### Jupiter vs Bags for Buying Tokens:
- Use **jupiter_buy_token** for any token including pump.fun memecoins (e.g. ends in `pump`)
- Use **buy_token** (Bags) for tokens listed on Bags.fm
- When in doubt, prefer **jupiter_buy_token** — it routes across all Solana DEXes
- Always call **jupiter_get_quote** first to preview price impact
...
class SolanaAgent:
    ...
    async def initialize(self):
        ...
        # Initialize Jupiter client (optional — falls back gracefully if no key)
        jupiter_api_key = getattr(self.config.solana, "bags_api_key", None)  # reuse or read from env
        try:
            import os
            jupiter_api_key = os.getenv("JUPITER_API_KEY") or jupiter_api_key
        except Exception:
            pass
        self.jupiter_client = JupiterClient(
            api_key=jupiter_api_key,
            wallet_pubkey=self.config.wallet.address,
            keypair=self.bags_client.keypair if self.bags_client else None,
        )
        print(f"{Colors.DIM}   → Jupiter Ultra client ready{Colors.RESET}")
