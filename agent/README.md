...
| **Agent staking**    | [solanaclawd.com/agents/stake](https://solanaclawd.com/agents/stake)          | live (MPL Core FreezeDelegate) |
...
- **Valid `$schema`**: [`https://solanaclawd.com/schemas/clawdAgentSchema.v1.json`](./schema/clawdAgentSchema.v1.json) applied across all schema-backed agents.

### New this release

- **Catalog + deploy flow** — [`agents-catalog.json`](./agents-catalog.json) aggregates all 53 agents with per-agent Install / Chat / Mint URLs. Served via `GET /api/agents/catalog` and rendered at [/agents](https://solanaclawd.com/agents) with one-shot badges, featured rail, category chips, and Metaplex capability filters.
