...
try:
    from .adapters.solana import SolanaTradingAdapter
...
class RalphOrchestrator:
    ...
    _KNOWN_ADAPTERS = ("solana-trading", "acp", "claude", "gemini", "qchat", "kiro")
    ...
    def __init__(
        ...
        if hasattr(prompt_file_or_config, 'prompt_file'):
            ...
            self.skills_dir = getattr(config, 'skills_dir', None)
            self.max_skills_detail = getattr(config, 'max_skills_detail', 3)
        ...
        token_address = getattr(self._config, 'token_address', None) if self._config else None
        skills_dir_val = self.skills_dir
        if skills_dir_val and isinstance(skills_dir_val, (str, Path)):
            skills_dir_val = Path(skills_dir_val)
        self.context_manager = ContextManager(
            self.prompt_file,
            prompt_text=self.prompt_text,
            token_address=token_address,
            skills_dir=skills_dir_val,
            max_skills_detail=self.max_skills_detail,
        )
        ...
        # TUI mode — emits JSONL events on stdout for tui.py to consume
        self.tui_mode = False
    ...
    def _emit(self, payload: Dict[str, Any]) -> None:
        """Emit a JSONL event on stdout when tui_mode is active."""
    ...
    def _initialize_adapters(self) -> Dict[str, ToolAdapter]:
        ...
        for name in desired_order:
            ...
            if name == "solana-trading":
                if SolanaTradingAdapter is None:
                    logger.warning("Solana trading adapter unavailable: optional dependencies not installed")
                    continue
                try:
                    # Pull config from ralph.yml adapters.solana section
                    default_token = getattr(self._config, 'token_address', None) or os.getenv("DEFAULT_TOKEN_MINT", "")
                    dry_run = os.getenv("DRY_RUN", "").lower() == "true"
                    trade_mode = os.getenv("TRADE_MODE", "moderate")

                    adapter = SolanaTradingAdapter(
                        default_token_mint=default_token,
                        dry_run=dry_run,
                        config=self._config.get_adapter_config("solana") if self._config and hasattr(self._config, "get_adapter_config") else None,
                    )
                    if adapter.available:
                        adapters["solana-trading"] = adapter
                        logger.info("Solana trading adapter initialized (token=%s, mode=%s, dry_run=%s)",
                                    default_token or "none", trade_mode, dry_run)
                    else:
                        logger.warning("Solana trading adapter not available (check JUPITER_API_KEY, wallet, and DEFAULT_TOKEN_MINT)")
                except Exception as e:
                    logger.warning(f"Solana trading adapter error: {e}")
                continue
    ...
    def _resolve_agent_priority(self) -> list[str]:
        ...
        for n in ("solana-trading", "claude", "kiro", "qchat", "gemini", "acp"):
            if n not in seen:
                normalized.append(n)
                seen.add(n)
    ...
    async def arun(self) -> None:
        ...
        self._emit({
            "event": "start",
            "agent": self.primary_tool,
            "prompt_file": str(self.prompt_file),
            "max_iterations": self.max_iterations,
            "max_runtime": self.max_runtime,
            "max_cost": self.max_cost if self.track_costs else None,
        ...
        while not self.stop_requested:
            ...
            self._emit({
                ...
                "cost": self.cost_tracker.total_cost if self.cost_tracker else 0.0,
            })
            ...
            # Emit per-iteration done event
            output_preview = ""
            if self.last_response_output:
                output_preview = self.last_response_output[:200]
            self._emit({
            ...
            if loop_detected:
                self._emit({"event": "loop_detected", "iteration": self.metrics.iterations})
                break
    ...
    def set_trading_broadcast_callback(self, callback):
        """Wire a callback to the SolanaTradingAdapter so each iteration tick is broadcast."""
        for name, adapter in self.adapters.items():
            if name == "solana-trading" and hasattr(adapter, "set_broadcast_callback"):
                adapter.set_broadcast_callback(callback)
                logger.info("Trading broadcast callback registered for %s", name)
