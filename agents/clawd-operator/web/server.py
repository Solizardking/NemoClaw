...
class OrchestratorMonitor:
    ...
    def register_orchestrator(self, orchestrator_id: str, orchestrator: RalphOrchestrator):
        ...
        # Wire the solana-trading adapter's broadcast callback to WebSocket clients
        if hasattr(orchestrator, "set_trading_broadcast_callback"):
            def _on_trading_tick(tick_data):
                """Called after each trading adapter iteration tick."""
                self._schedule_broadcast({
                    "type": "trading_tick",
                    "data": tick_data,
                })
            orchestrator.set_trading_broadcast_callback(_on_trading_tick)
            logger.info("Trading broadcast callback wired for orchestrator %s", orchestrator_id)
        else:
            # Fallback: manually wire if solana-trading adapter exists
            for name, adapter in orchestrator.adapters.items():
                if name == "solana-trading" and hasattr(adapter, "set_broadcast_callback"):
                    def _on_trading_tick(tick_data):
                        self._schedule_broadcast({
                            "type": "trading_tick",
                            "data": tick_data,
                        })
                    adapter.set_broadcast_callback(_on_trading_tick)
                    logger.info("Trading broadcast callback wired (direct) for %s", orchestrator_id)
...
class WebMonitor:
    ...
    def _setup_routes(self):
        ...
        @self.app.get("/dark-ralph")
        async def dark_ralph_dashboard():
            """Serve the Dark Ralph live trading dashboard."""
            static_dir = Path(__file__).parent / "static"
            dark_ralph_file = static_dir / "dark-ralph.html"
            if not dark_ralph_file.exists():
                raise HTTPException(status_code=404, detail="dark-ralph.html not found in static/")
            return FileResponse(str(dark_ralph_file))

        @self.app.get("/api/trading/ticks", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_ticks(limit: int = 50):
            """Return recent ticks from live SolanaTradingAdapter iteration_log."""
            ticks = []
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    ticks = adapter.iteration_log[-limit:]
                    break
            return {"ticks": ticks, "count": len(ticks)}

        @self.app.get("/api/trading/iteration_log", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_iteration_log(limit: int = 100):
            """Return the trading adapter's full iteration log."""
            all_ticks = []
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    all_ticks.extend(adapter.iteration_log[-limit:])
            return {"iteration_log": all_ticks, "count": len(all_ticks)}

        # ------------------------------------------------------------------
        # Solana Trading API endpoints
        # ------------------------------------------------------------------

        @self.app.get("/api/trading/status", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_status():
            """Get overall Solana trading adapter status from active orchestrators."""
            results = []
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    results.append({
                        ...
                        "wallet": adapter.wallet_pubkey,
                        "default_token": adapter.default_token_mint,
                        "mode": adapter.strategy.mode.value,
                        "dry_run": adapter.dry_run,
                        "total_trades": adapter.total_trades,
                        "successful_trades": adapter.successful_trades,
                        "failed_trades": adapter.failed_trades,
                        "total_pnl_usd": round(adapter.total_pnl_usd, 4),
                        "open_positions": len(adapter.positions),
                        "available": adapter.available,
                    })
            return {"trading_adapters": results, "count": len(results)}

        @self.app.get("/api/trading/positions", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_positions():
            """Get all open trading positions from active orchestrators."""
            positions = []
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    for mint, pos in adapter.positions.items():
                        positions.append({
                            "orchestrator_id": orch_id,
                            "token_mint": mint,
                            "token_symbol": pos.token_symbol,
                            "entry_price_usd": pos.entry_price,
        ...
        @self.app.get("/api/trading/history", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_history(limit: int = 50):
            """Get trade history from active orchestrators."""
            trades = []
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    for entry in adapter.trade_history[-limit:]:
                        trades.append({
                            "orchestrator_id": orch_id,
                            ...
                            "pnl_pct": round(entry.pnl_pct, 2) if entry.pnl_pct is not None else None,
                        })
            return {"trades": trades, "count": len(trades)}

        @self.app.get("/api/trading/summary", dependencies=[auth_dependency] if self.enable_auth else [])
        async def get_trading_summary():
            """Get aggregated trading summary across all orchestrators."""
            total_trades = 0
            total_successful = 0
            total_failed = 0
            total_pnl = 0.0
            ...
            for orch_id, orch in self.monitor.active_orchestrators.items():
                if "solana-trading" in orch.adapters:
                    adapter = orch.adapters["solana-trading"]
                    total_trades += adapter.total_trades
                    total_successful += adapter.successful_trades
                    total_failed += adapter.failed_trades
                    total_pnl += adapter.total_pnl_usd
                    total_positions += len(adapter.positions)
                    modes.add(adapter.strategy.mode.value)
                    if adapter.dry_run:
                        dry_runs += 1
            ...
            return {
                ...
                "orchestrator_count": len(self.monitor.active_orchestrators),
            }
