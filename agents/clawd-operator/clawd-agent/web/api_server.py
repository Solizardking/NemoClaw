...
# ============================================================
# Birdeye REST API Endpoints (for chart, overview, trends)
# ============================================================

_birdeye_http_client = None
_birdeye_http_lock = asyncio.Lock()

async def get_birdeye_http_client():
    """Get or create the Birdeye HTTP client"""
    global _birdeye_http_client
    async with _birdeye_http_lock:
        if _birdeye_http_client is None:
            ...
            if not env_path.exists():
                env_path = Path(__file__).parent.parent / ".env.local"
            config = load_root_config(str(env_path) if env_path.exists() else None)
            _birdeye_http_client = BirdeyeClient(api_key=config.birdeye_api_key)
        return _birdeye_http_client
...
@app.get("/api/birdeye/ohlcv/{token_address}")
async def birdeye_ohlcv(token_address: str, type: str = Query("15m", description="1m,3m,5m,15m,30m,1H,2H,4H,6H,8H,12H,1D"), time_from: Optional[int] = Query(None), time_to: Optional[int] = Query(None)):
    """Get OHLCV candlestick data from Birdeye"""
    client = await get_birdeye_http_client()
    be_type = _BIRDEYE_TF_MAP.get(type.lower(), type)
    if time_to is None:
        time_to = int(_time_module.time())
    if time_from is None:
        # Default to 24h of data
        time_from = time_to - 86400 * 3  # 3 days
    data = await client.get_ohlcv(token_address, be_type, time_from, time_to)
    return {"success": True, "data": {"items": [d.__dict__ if hasattr(d, '__dict__') else d for d in data]}}

@app.get("/api/birdeye/overview/{token_address}")
async def birdeye_overview(token_address: str):
    """Get token overview from Birdeye"""
    client = await get_birdeye_http_client()
    overview = await client.get_token_overview(token_address)
    if hasattr(overview, '__dict__'):
        return {"success": True, "data": overview.__dict__}
    return {"success": True, "data": overview}

@app.get("/api/birdeye/trending")
async def birdeye_trending(
    sort_by: str = Query("v24hUSD", description="v24hUSD, mc, liquidity, v24hChangePercent"),
    sort_type: str = Query("desc"),
    limit: int = Query(20),
):
    """Get trending/top tokens from Birdeye"""
    client = await get_birdeye_http_client()
    tokens = await client.get_trending_tokens(sort_by=sort_by, sort_type=sort_type, limit=limit)
    return {"success": True, "tokens": tokens}

@app.get("/api/birdeye/top-performers/{timeframe}")
async def birdeye_top_performers(timeframe: str = "24h", limit: int = Query(20)):
    """Get top gainers/losers from Birdeye"""
    client = await get_birdeye_http_client()
    tokens = await client.get_gainers_losers(time_type=timeframe, sort_type="desc", limit=limit)
    return {"success": True, "tokens": tokens}

@app.get("/api/birdeye/price/{token_address}")
async def birdeye_price(token_address: str):
...
# ============================================================
# Solana Tracker Data API Endpoints
# ============================================================


# Lazy-initialized global tracker client
_tracker_client = None
_tracker_lock = asyncio.Lock()

async def get_tracker_client():
    """Get or create the Solana Tracker client"""
    global _tracker_client
    async with _tracker_lock:
        if _tracker_client is None:
            from clients.solana_tracker_client import SolanaTrackerClient
            from config import load_config as load_root_config
            env_path = Path(__file__).parent.parent.parent / ".env.local"
            if not env_path.exists():
                env_path = Path(__file__).parent.parent / ".env.local"
            ...
            _tracker_client = SolanaTrackerClient(api_key=api_key)
        return _tracker_client

@app.get("/api/tracker/price/{token_address}")
async def tracker_get_price(token_address: str):
    """Get current token price"""
    client = await get_tracker_client()
    return await client.get_token_price(token_address)

@app.get("/api/tracker/price/history/{token_address}")
async def tracker_price_history(token_address: str, timeframe: str = Query("1h", description="1m,5m,15m,30m,1h,2h,4h,6h,8h,12h,24h")):
    """Get historic price information"""
    client = await get_tracker_client()
    return await client.get_price_history(token_address, timeframe)

@app.get("/api/tracker/price/multi")
async def tracker_multi_prices(tokens: str = Query(..., description="Comma-separated token addresses")):
    """Get prices for multiple tokens"""
    client = await get_tracker_client()
    token_list = [t.strip() for t in tokens.split(",") if t.strip()]
    return await client.get_multi_token_prices(token_list)

@app.get("/api/tracker/ohlcv/{token_address}")
async def tracker_ohlcv(token_address: str, timeframe: str = Query("1h", description="1m,3m,5m,15m,30m,1h,2h,4h,6h,8h,12h,1d")):
    """Get OHLCV chart data for a token"""
    client = await get_tracker_client()
    return await client.get_ohlcv(token_address, timeframe)

@app.get("/api/tracker/overview")
async def tracker_overview(tokens: str = Query(..., description="Comma-separated token addresses")):
    """Get overview for one or more tokens"""
    client = await get_tracker_client()
    token_list = [t.strip() for t in tokens.split(",") if t.strip()]
    return await client.get_token_overview(token_list)

@app.get("/api/tracker/trending")
async def tracker_trending():
    """Get trending tokens"""
    client = await get_tracker_client()
    return await client.get_trending_tokens()

@app.get("/api/tracker/trending/{timeframe}")
async def tracker_trending_timeframe(timeframe: str):
    """Get trending tokens by timeframe"""
    client = await get_tracker_client()
    return await client.get_trending_tokens_by_timeframe(timeframe)

@app.get("/api/tracker/top-performers/{timeframe}")
async def tracker_top_performers(timeframe: str = "24h"):
    """Get top performing tokens"""
    client = await get_tracker_client()
    return await client.get_top_performers(timeframe)

@app.get("/api/tracker/stats/{token_address}")
async def tracker_stats(token_address: str):
    """Get token statistics"""
    client = await get_tracker_client()
    return await client.get_token_stats(token_address)

@app.get("/api/tracker/events/{token_address}")
async def tracker_events(token_address: str):
    """Get token events"""
    client = await get_tracker_client()
    return await client.get_token_events(token_address)

@app.get("/api/tracker/trades/{token_address}")
async def tracker_trades(token_address: str, limit: int = Query(50), offset: int = Query(0)):
    """Get trades for a token"""
    client = await get_tracker_client()
    return await client.get_token_trades(token_address, limit, offset)

@app.get("/api/tracker/first-buyers/{token_address}")
async def tracker_first_buyers(token_address: str, limit: int = Query(50)):
    """Get first buyers of a token"""
    client = await get_tracker_client()
    return await client.get_first_buyers(token_address, limit)

@app.get("/api/tracker/bundlers/{token_address}")
async def tracker_bundlers(token_address: str):
    """Get token bundlers (cohort wallets)"""
    client = await get_tracker_client()
    return await client.get_token_bundlers(token_address)

@app.get("/api/tracker/holders/{token_address}")
async def tracker_holders(token_address: str, page: int = Query(1), limit: int = Query(50)):
    """Get paginated token holders"""
    client = await get_tracker_client()
    return await client.get_holders_paginated(token_address, page, limit)

@app.get("/api/tracker/chart/holders/{token_address}")
async def tracker_holders_chart(token_address: str):
    """Get holders chart data"""
    client = await get_tracker_client()
    return await client.get_holders_chart(token_address)

@app.get("/api/tracker/chart/bundlers/{token_address}")
async def tracker_bundlers_chart(token_address: str):
    """Get bundlers chart data"""
    client = await get_tracker_client()
    return await client.get_bundlers_chart(token_address)

@app.get("/api/tracker/wallet/{wallet_address}")
async def tracker_wallet_tokens(wallet_address: str):
    """Get wallet tokens"""
    client = await get_tracker_client()
    return await client.get_wallet_tokens(wallet_address)

@app.get("/api/tracker/wallet/{wallet_address}/basic")
async def tracker_wallet_basic(wallet_address: str):
    """Get basic wallet info"""
    client = await get_tracker_client()
    return await client.get_wallet_basic(wallet_address)

@app.get("/api/tracker/wallet/{wallet_address}/trades")
async def tracker_wallet_trades(wallet_address: str, limit: int = Query(50), offset: int = Query(0)):
    """Get wallet trade history"""
    client = await get_tracker_client()
    return await client.get_wallet_trades(wallet_address, limit, offset)

@app.get("/api/tracker/wallet/{wallet_address}/chart")
async def tracker_wallet_chart(wallet_address: str):
    """Get wallet portfolio chart"""
    client = await get_tracker_client()
    return await client.get_wallet_chart(wallet_address)

@app.get("/api/tracker/pnl/{wallet_address}")
async def tracker_wallet_pnl(wallet_address: str):
    """Get wallet PnL"""
    client = await get_tracker_client()
    return await client.get_wallet_pnl(wallet_address)

@app.get("/api/tracker/pnl/{wallet_address}/{token_address}")
async def tracker_token_pnl(wallet_address: str, token_address: str):
    """Get token-specific PnL for a wallet"""
    client = await get_tracker_client()
    return await client.get_token_pnl(wallet_address, token_address)

@app.get("/api/tracker/top-traders")
async def tracker_top_traders():
    """Get top traders globally"""
    client = await get_tracker_client()
    return await client.get_top_traders()

@app.get("/api/tracker/top-traders/{token_address}")
async def tracker_top_traders_token(token_address: str):
    """Get top traders for a token"""
    client = await get_tracker_client()
    return await client.get_top_traders_for_token(token_address)
...
async def serve_chart():
    """Serve the chart/DEX interface"""
    chart_path = frontend_path / "chart.html"
    if chart_path.exists():
        return FileResponse(str(chart_path))
    return {"message": "Chart interface not found"}
