...
class PumpFunClient:
    ...
    async def buy(
        ...
        # Detect token program (legacy SPL vs Token2022) from the mint's owner.
        # Mayhem / create_v2 coins are owned by Token2022.
        token_program = await self.get_mint_token_program(mint)

        # Pick fee recipient: Mayhem coins require a Mayhem fee recipient at
        # account index 1. Legacy coins continue to use PUMP_FEE_RECIPIENT.
        fee_recipient = (
            self.pick_mayhem_fee_recipient(mint)
            if state.is_mayhem_mode else PUMP_FEE_RECIPIENT
        )

        # Build and send transaction
        bonding_curve = self.get_bonding_curve_pda(mint)
        associated_bonding_curve = self.get_associated_token_address(
            bonding_curve, mint, token_program=token_program
        )
    ...
    async def sell(
        ...
        # Detect token program + Mayhem fee recipient (see buy() above)
        token_program = await self.get_mint_token_program(mint)
        fee_recipient = (
            self.pick_mayhem_fee_recipient(mint)
            if state.is_mayhem_mode else PUMP_FEE_RECIPIENT
