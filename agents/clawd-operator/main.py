...
class RalphConfig:
    ...
    # Solana token target
    token_address: Optional[str] = None  # Solana mint address for the target token

    # Skill registry
    skills_dir: Optional[str] = "skills"  # Path to skills/ directory
    max_skills_detail: int = 3  # Skills with full docs in prompt (0=compact, -1=all)
    ...
    def from_yaml(cls, config_path: str) -> 'RalphConfig':
        ...
        if 'adapters' in config_data:
            adapter_configs = {}
            adapter_valid_keys = {f.name for f in AdapterConfig.__dataclass_fields__.values()}
            for name, adapter_data in config_data['adapters'].items():
                if isinstance(adapter_data, dict):
                    known_adapter_data = {k: v for k, v in adapter_data.items() if k in adapter_valid_keys}
                    extra_adapter_data = {k: v for k, v in adapter_data.items() if k not in adapter_valid_keys}
                    adapter_configs[name] = AdapterConfig(
                        **known_adapter_data,
                        extra=extra_adapter_data,
                    )
                else:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Solana token mint address to focus on (injected into prompt context)"
    )
    ...
    parser.add_argument(
        "--tui",
        action="store_true",
        help="Emit JSONL events on stdout for tui.py to render (pipe: python -m clawd_operator | python tui.py)"
    )
