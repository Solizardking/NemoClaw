...
exports.detectHostClawd = detectHostClawd;
...
function resolveStateDir(env = process.env) {
    ...
    return node_path_1.default.join(resolveHostHome(env), ".clawd");
}
function resolveConfigPath(stateDir, env = process.env) {
    ...
    return node_path_1.default.join(stateDir, "clawd.json");
}
...
function defaultWorkspacePath(env = process.env) {
    ...
    if (profile && profile.toLowerCase() !== "default") {
        return node_path_1.default.join(home, ".clawd", `workspace-${profile}`);
    }
    return node_path_1.default.join(home, ".clawd", "workspace");
}
...
function detectHostClawd(env = process.env) {
...
function resolveConfigSourcePath(manifest, snapshotDir) {
    if (manifest.hasExternalConfig) {
        return node_path_1.default.join(snapshotDir, "config", "clawd.json");
    }
    return node_path_1.default.join(snapshotDir, "clawd", "clawd.json");
}
...
function prepareSandboxState(snapshotDir, manifest) {
    const preparedStateDir = node_path_1.default.join(snapshotDir, "sandbox-bundle", "clawd");
    ...
    copyDirectory(node_path_1.default.join(snapshotDir, "clawd"), preparedStateDir);
    ...
    (0, node_fs_1.writeFileSync)(node_path_1.default.join(preparedStateDir, "clawd.json"), JSON.stringify(config, null, 2));
...
function createSnapshotBundle(hostState, logger, options) {
    ...
    try {
        ...
        const snapshotStateDir = node_path_1.default.join(parentDir, "clawd");
        ...
        if (hostState.configPath && hostState.hasExternalConfig) {
            ...
            (0, node_fs_1.copyFileSync)(hostState.configPath, node_path_1.default.join(configSnapshotDir, "clawd.json"));
        }
        const externalRoots = [];
...
function restoreSnapshotToHost(snapshotDir, logger) {
    ...
    const snapshotStateDir = node_path_1.default.join(snapshotDir, "clawd");
    ...
    try {
        ...
        if (manifest.hasExternalConfig && manifest.configPath) {
            const configSnapshotPath = node_path_1.default.join(snapshotDir, "config", "clawd.json");
