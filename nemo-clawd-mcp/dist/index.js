...
async function grokChat(messages, model = "grok-4.20-reasoning", stream = false) {
    if (!XAI_API_KEY) {
...
async function handleToolCall(name, args) {
    switch (name) {
        ...
        case "solana_price": {
            ...
            const response = await fetch(`https://api.birdeye.so/public/v1/token/${token}?api_key=${BIRDEYE_API_KEY || ""}`);
            const data = asJsonObject(await response.json());
        ...
        case "helius_balance": {
            const info = await heliusRpc("getBalance", [args.address]);
