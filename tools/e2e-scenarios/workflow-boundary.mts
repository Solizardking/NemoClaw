...
function validateOpenShellGatewayAuthContractVitestJob(
  ...
  if (job["runs-on"] !== "ubuntu-latest") {
    errors.push("openshell-gateway-auth-contract-vitest job must run on ubuntu-latest");
  }
  if (job.needs !== "generate-matrix") {
    errors.push("openshell-gateway-auth-contract-vitest job must depend on generate-matrix");
  }
  if (job.if !== explicitOnlyFreeStandingJobIf(jobName, "openshell-gateway-auth-contract")) {
    errors.push(
  ...
  const jobEnv = asRecord(job.env);
  if (jobEnv.NEMOCLAW_RUN_E2E_SCENARIOS !== "1") {
    errors.push("openshell-gateway-auth-contract-vitest job must set NEMOCLAW_RUN_E2E_SCENARIOS=1");
  }
  if (jobEnv.NEMOCLAW_OPENSHELL_PIN_VERSION !== "0.0.72") {
    errors.push(
      "openshell-gateway-auth-contract-vitest job must pin NEMOCLAW_OPENSHELL_PIN_VERSION=0.0.72",
    );
  }
  if (
  ...
  const checkout = steps.find((step) => stringValue(step.uses).startsWith("actions/checkout@"));
  if (!checkout) {
    errors.push("openshell-gateway-auth-contract-vitest job missing checkout step");
  }
  requireFullShaAction(errors, checkout, "openshell-gateway-auth-contract-vitest checkout");
  if (asRecord(checkout?.with)["persist-credentials"] !== false) {
    errors.push(
  ...
  const setupNode = namedStep(steps, "Set up Node");
  if (!setupNode) {
    errors.push("openshell-gateway-auth-contract-vitest job missing step: Set up Node");
  }
  requireFullShaAction(errors, setupNode, "openshell-gateway-auth-contract-vitest setup-node");
  ...
  requireRunContains(errors, runVitest, 'OPENSHELL_GATEWAY_BIN="$(command -v openshell-gateway)"');
  ...
  requireFullShaAction(errors, upload, "openshell-gateway-auth-contract-vitest upload-artifact");
  const uploadWith = asRecord(upload?.with);
  if (uploadWith.name !== "e2e-vitest-scenarios-openshell-gateway-auth-contract") {
    errors.push("openshell-gateway-auth-contract-vitest artifact upload name must be stable");
  }
  const uploadPath = stringValue(uploadWith.path);
  ...
  if (uploadWith["retention-days"] !== 14) {
    errors.push("openshell-gateway-auth-contract-vitest artifact upload retention-days must be 14");
  }
}
...
function validateNetworkPolicyVitestJob(errors: string[], jobs: WorkflowRecord): void {
  ...
  if (jobEnv.NEMOCLAW_OPENSHELL_CHANNEL !== "${{ inputs.openshell_channel }}") {
    errors.push("network-policy-vitest job must pass openshell_channel to install-openshell.sh");
  }
  for (const secret of [
    "NVIDIA_INFERENCE_API_KEY",
    "DOCKERHUB_USERNAME",
...
function validateMcpBridgeVitestJob(errors: string[], jobs: WorkflowRecord): void {
  ...
  if (jobEnv.NEMOCLAW_MCP_BRIDGE_AGENT_MATRIX !== "1") {
    errors.push("mcp-bridge-vitest job must exercise the MCP bridge agent matrix");
  }
  if (jobEnv.E2E_ARTIFACT_DIR !== "${{ github.workspace }}/e2e-artifacts/vitest/mcp-bridge") {
    errors.push("mcp-bridge-vitest job must write artifacts under e2e-artifacts/vitest/mcp-bridge");
  }
  if (!stringValue(jobEnv.NEMOCLAW_CLI_BIN).includes("bin/nemoclaw.js")) {
    errors.push("mcp-bridge-vitest job must point NEMOCLAW_CLI_BIN at the repo CLI");
  }
  if (jobEnv.NEMOCLAW_OPENSHELL_CHANNEL !== "${{ inputs.openshell_channel }}") {
    errors.push("mcp-bridge-vitest job must pass openshell_channel to install-openshell.sh");
  }
  for (const secret of [
    "NVIDIA_INFERENCE_API_KEY",
    "DOCKERHUB_USERNAME",
  ...
  if (authEnv.DOCKERHUB_USERNAME !== "${{ secrets.DOCKERHUB_USERNAME }}") {
    errors.push("mcp-bridge-vitest Docker auth step must receive DOCKERHUB_USERNAME from secrets");
  }
  if (authEnv.DOCKERHUB_TOKEN !== "${{ secrets.DOCKERHUB_TOKEN }}") {
    errors.push("mcp-bridge-vitest Docker auth step must receive DOCKERHUB_TOKEN from secrets");
  }
  ...
  const runVitest = requireJobStep(errors, jobName, steps, "Run MCP OpenShell provider live test");
  requireRunContains(
    errors,
    runVitest,
    'export PATH="$HOME/.local/bin:$HOME/.npm-global/bin:$PATH"',
  ...
  requireRunContains(errors, runVitest, "export OPENSHELL_BIN");
  requireRunContains(errors, runVitest, "npx vitest run --project e2e-scenarios-live");
  requireRunContains(errors, runVitest, "test/e2e-scenario/live/mcp-bridge.test.ts");
  ...
  if (uploadWith["if-no-files-found"] !== "ignore") {
    errors.push("mcp-bridge-vitest artifact upload must ignore missing fixture artifacts");
  }
  if (uploadWith["retention-days"] !== 14) {
...
function validateGatewayDriftPreflightVitestJob(errors: string[], jobs: WorkflowRecord): void {
...
export function validateE2eVitestScenariosWorkflowBoundary(
  ...
  const openshellChannelInput = requireInput(errors, dispatchInputs, "openshell_channel");
  ...
  if (workflowEnv.NEMOCLAW_OPENSHELL_CHANNEL !== "${{ inputs.openshell_channel }}") {
    errors.push("workflow env must propagate openshell_channel to the entire E2E fan-out");
  }
