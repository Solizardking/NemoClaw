...
import { MCP_BRIDGE_TEST_CREDENTIALS } from "../../test/e2e-scenario/fixtures/mcp-bridge-credentials.ts";
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    if (!root || process.argv.length !== 3) {
      throw new Error(
        "Usage: npx tsx tools/e2e-scenarios/assert-mcp-artifact-secrets-absent.mts ARTIFACT_DIR",
      );
    }
