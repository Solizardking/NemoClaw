...
- `scripts/docs-to-skills.py` generates skills from `docs/` into `.agents/skills/`. The generated skills are: `nemoclaw-configure-inference`, `nemoclaw-configure-security`, `nemoclaw-deploy-remote`, `nemoclaw-get-started`, `nemoclaw-manage-policy`, `nemoclaw-monitor-sandbox`, `nemoclaw-overview`, `nemoclaw-reference`, `nemoclaw-security-best`, `nemoclaw-workspace`. Never edit these directly — change the source in `docs/` and regenerate:

  ```bash
  python3 scripts/docs-to-skills.py docs/ .agents/skills/ --prefix nemoclaw
  ```

- All other skills (e.g., `nemoclaw-maintainer-*`, `cut-release-tag`, `find-review-pr`, `security-code-review`) are hand-authored. Edit them directly.
