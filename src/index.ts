...
export interface PluginCliCommand {
  command(name: string): PluginCliCommand;
  description(text: string): PluginCliCommand;
  option(flag: string, description: string, defaultValue?: unknown): PluginCliCommand;
  action(handler: (...args: any[]) => unknown): PluginCliCommand;
}

export interface PluginCliContext {
  program: PluginCliCommand;
  config: NemoclawdConfig;
  workspaceDir?: string;
  logger: PluginLogger;
}
...
export default function register(api: NemoclawdPluginApi): void {
  ...
  // 2. Register `nemoclawd` CLI subcommands (commander.js)
  api.registerCli(
    (cliCtx) => {
  ...
  api.logger.info("  │  Commands:  nemoclawd <command>                         │");
  api.logger.info("  └─────────────────────────────────────────────────────┘");
  api.logger.info("");
}
