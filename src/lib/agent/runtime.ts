...
export function buildHermesManagedGatewayProbe(): string {
  const probe = [
    ...
    "managed = len(managers) == 1 and len(gateways) == 1 and parent_pid(gateways[0]) == managers[0]",
...
function buildHermesServiceManagerShutdown(): string {
  const shutdown = [
    ...
    "except OSError:",
    "    pids = []",
    "identities = [(pid, start_time(pid)) for pid in pids if pid not in {os.getpid(), os.getppid()} and is_manager(read_argv(pid))]",
