...
- `src/lib/onboard/docker-driver-gateway-launch.test.ts` verifies loopback main binding, digest-pinned compatibility image selection, wildcard override rejection, stale auth-disable env scrubbing, generated `OPENSHELL_GATEWAY_CONFIG`, local mTLS config, and Docker `guest_tls_*` propagation.
