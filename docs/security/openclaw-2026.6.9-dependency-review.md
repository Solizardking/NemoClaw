...
- `src/lib/actions/sandbox/rebuild-resume-config.test.ts` is unchanged from current `main`; the rebuild-reasoning cases added by this PR live in the focused `rebuild-resume-reasoning.test.ts` file instead of growing that existing 468-line suite.
...
- The shared archive-installer redesign remains explicitly deferred to issue #5896. Consolidating the reviewed archive helper would change the Codex ACP, OpenClaw core, base-image, optional-plugin, and messaging installation boundaries together; this PR keeps each SRI-bound copy fail closed and covered while avoiding that cross-installer migration.
...
- Live gateway display output is treated as untrusted text: provider/model values must be present, bounded, and syntax-safe before recovery, and provider metadata must contain exactly one expected credential key and endpoint-config key. Partial or ambiguous output fails closed.
- Retained older OpenClaw pins are inactive compatibility/rollback branches, not the production default. The production-build argument guard requires the active `2026.6.9` pin and SRI before every production image build.
