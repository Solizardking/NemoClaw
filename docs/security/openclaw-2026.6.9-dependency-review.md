...
NemoClaw enforces the main `openclaw@2026.6.9`, `@zed-industries/codex-acp@0.11.1`, and each reviewed npm plugin registry integrity, including optional OTEL/brave plugins and messaging plugins, before running `npm install` or `openclaw plugins install`. The Codex ACP helper is also installed from the reviewed npm tarball URL after checking that the package metadata still reports that URL.
...
The committed npm SRI checks are registry-drift gates, not lockfile-style artifact binding. `Dockerfile`, `Dockerfile.base`, and the messaging build applier verify the reviewed `dist.integrity` for `openclaw@2026.6.9`, optional OpenClaw plugins, and built-in messaging OpenClaw plugins before install, then `npm install -g` or `openclaw plugins install` performs its own registry transaction. `Dockerfile` verifies both `dist.integrity` and `dist.tarball` for `@zed-industries/codex-acp@0.11.1`, then installs the helper from `https://registry.npmjs.org/@zed-industries/codex-acp/-/codex-acp-0.11.1.tgz` instead of a floating package spec.

Accepted residual risk: a compromised or inconsistent registry mirror could return the reviewed SRI for the OpenClaw/plugin preflight lookup and different install metadata later. This PR accepts that residual installer-trust risk for the OpenClaw 2026.6.9 bump because the change is a dependency-version compatibility update, not an installer architecture migration, and because the current guard still fails closed on missing or changed reviewed SRI before any install starts. Do not treat the OpenClaw/plugin checks as proving lockfile-enforced or tarball-enforced artifact pinning.

Invalid state: claiming the committed SRI pins bind every later OpenClaw/plugin install transaction. Source boundary: Dockerfile npm install blocks, `Dockerfile.base`, optional plugin install blocks, and `src/lib/messaging/applier/build/messaging-build-applier.mts`. Source-fix constraint: robust OpenClaw/plugin artifact binding requires a lockfile, reviewed tarball path, or post-install artifact verifier that enforces the committed SRI against the installed artifact. Regression test: `test/openclaw-integrity-pin.test.ts` verifies fail-closed pre-install drift detection, the Codex ACP tarball URL pin, and this accepted residual-risk note. Removal condition: replace the OpenClaw/plugin preflight-only pattern with artifact-bound installation and add a split-registry fake test where `npm view` returns the reviewed SRI but the later install receives a different artifact.
...
- The npm audit result in this note is a manual snapshot for the reviewed lock-only graph. It is not a new CI gate; rerun the command in the Advisory Check section on the next OpenClaw/plugin bump or if npm advisory state changes before merge.
- The stale nonterminal rebuild-resume repair in `src/lib/actions/sandbox/rebuild-resume-session.ts` remains a migration compatibility shim. Its removal condition is to delete it after supported persisted sessions can no longer contain pre-repair nonterminal snapshots; `src/lib/actions/sandbox/rebuild-resume-snapshot.test.ts` owns the regression.
