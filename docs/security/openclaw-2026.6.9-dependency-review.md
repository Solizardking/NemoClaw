...
Merge disposition for this OpenClaw 2026.6.9 bump: #4434 remains a partial acceptance item, not a full issue closure. This PR may merge only if release notes or merge context keep that partial-scope label attached and no PR text claims full #4434 completion. No additional code change is expected in this PR unless the current reviewed OpenClaw output starts emitting all three fields; in that case `test/issue-4434-error-fields.test.ts` is the trigger to tighten both live guards before merge.
