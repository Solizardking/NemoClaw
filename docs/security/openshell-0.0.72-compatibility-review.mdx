...
The live `network-policy` lane isolates that upstream contract from NemoClaw's MCP implementation.
It applies a raw OpenShell `protocol: mcp` policy with `allowed_ips: [1.1.1.1]`, remaps the hostname to a reachable private runner address, sends a raw MCP `tools/list` request, requires an exact HTTP 403, and verifies zero upstream requests without calling `nemoclaw mcp` or any agent adapter.
...
- The live network-policy lane independently proves raw OpenShell `allowed_ips` rebinding denial with an exact HTTP 403 and zero upstream requests.
