...
Built-in preset choices are scoped to the sandbox's active agent. Messaging channel presets such as `telegram`, `discord`, `slack`, `wechat`, and `whatsapp` apply only to agents that support those channels.
