...
Run `$$nemoclaw <name> shields down --timeout 5m --reason "MCP maintenance"` before the mutation, then run `$$nemoclaw <name> shields up` after it; list and status remain read-only.
