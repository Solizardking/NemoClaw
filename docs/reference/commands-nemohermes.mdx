...
You can also name the sandbox in sandbox-first position instead of passing `--sandbox`; `nemohermes <name> inference set --provider <provider> --model <model>` targets `<name>` directly and is equivalent to `nemohermes inference set --provider <provider> --model <model> --sandbox <name>`.
