...
All endpoints must use HTTPS, and persisted MCP URLs cannot contain query strings, percent-escaped or glob-style paths, or port zero.
OpenShell globally reserves the credential key for the generated policy, requires verified TLS and an exact Host match, then enforces the endpoint, runtime, path, query, and MCP-method boundary before replacing the placeholder; broader overlapping policies cannot widen that decision, and denied requests are neither rewritten nor sent upstream.
The sandbox client connects directly through OpenShell's existing egress path, and NemoClaw does not run a host-side MCP bridge, proxy, relay, or listener.
For full setup details, see [Set Up MCP Servers](../manage-sandboxes/set-up-mcp-servers).

```bash
export GITHUB_TOKEN=ghp_...
nemohermes my-assistant mcp add github --url https://api.githubcopilot.com/mcp/ --env GITHUB_TOKEN
unset GITHUB_TOKEN
```
...
Otherwise, restart reuses an existing exact-match provider.
...
NemoClaw unregisters the sandbox agent adapter, detaches and deletes the exact-match OpenShell provider, removes the owned generated policy, and clears the sandbox registry entry.
The command fails closed on drift. `--force` continues best-effort cleanup of independently provable resources, but never claims a non-matching provider or policy and preserves registry state when residuals remain.
...
| Flag | Description |
|------|-------------|
| `--force` | Best-effort cleanup of provably owned resources; preserves registry state when residuals remain |
