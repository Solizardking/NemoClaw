...
NemoClaw generates a narrow `protocol: mcp` policy for the destination, literal path, adapter binaries, pinned addresses, and explicit MCP methods. OpenShell current main evaluates that policy before replacing the attached provider placeholder in the allowed request header. Static provider placeholders are sandbox-scoped rather than endpoint-exclusive, so do not grant broader inspected-HTTP routes to the same adapter runtime and credential key.
...
The command fails closed on observed drift. `--force` continues best-effort cleanup of independently matching resources and preserves registry state when residuals remain. OpenShell current main mutates providers by name, so do not concurrently replace a managed provider through another OpenShell client during this command.
...
| Flag | Description |
|------|-------------|
| `--force` | Best-effort cleanup of resources whose current metadata match the registry; preserves registry state when residuals remain |
