...
Credentials stay in the host process environment: pass `--env KEY` to reference an existing host environment variable, or `--env KEY=VALUE` to use an inline value only for the initial proxy launch.
