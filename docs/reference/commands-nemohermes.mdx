...
Inline `--env KEY=VALUE` values are rejected so restart can relaunch from persisted environment variable names without storing raw API keys.
NemoClaw persists only the environment variable names, allocates a loopback bridge port, applies a generated OpenShell `protocol: mcp` network policy for `host.docker.internal:<port>`, and registers the endpoint with OpenClaw through `mcporter`.
