...
Use `openshell policy get --base <name>` and `openshell policy set --policy <file> --wait <name>` only when you need to edit and replace the round-trippable base policy. Use `--full` only to inspect the effective policy, including provider-composed rules.
