...
Hermes and Deep Agents probe their managed MCP runtime before provider or policy side effects; OpenClaw verifies the pinned `mcporter` during adapter registration and rolls back an incomplete add.
...
When Hermes shields are up, run `nemohermes <sandbox> shields down --timeout 5m --reason "MCP maintenance"` before `mcp add`, `mcp restart`, or `mcp remove`, then run `nemohermes <sandbox> shields up` after the change.
...
If an MCP mutation reports that `mcporter`, the Hermes transaction helper, or the managed Deep Agents MCP-aware launcher is unavailable, rebuild the sandbox onto a current image before retrying.
