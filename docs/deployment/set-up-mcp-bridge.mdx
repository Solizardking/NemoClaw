...
NemoClaw does not launch an MCP server, stdio adapter, bridge, credential proxy, relay, or listener on the host.
...
NemoClaw invokes the validated transaction helper as a one-shot `openshell sandbox exec` command.
The required OpenShell build places that command in the same workload uid and network namespace as Hermes, so the helper can update the same-uid compatibility hash, signal the exact gateway PID, and verify its loopback health.
The helper rejects non-root execution when the root-separated lifecycle marker or a separately owned gateway is present.
It validates the PID against the trusted Hermes gateway launcher before signaling it.
There is no persistent control socket or service.
