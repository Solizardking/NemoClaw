...
COPY nemoclawd/nemoclawd.plugin.json /opt/nemoclawd/
...
RUN chmod +x /usr/local/bin/nemoclawd-start
RUN chmod +x /usr/local/bin/nemoclawd-solana-agent
RUN chmod +x /usr/local/bin/nemoclawd-payment-app
RUN chmod +x /usr/local/bin/nemoclawd-telegram-bot
RUN chmod +x /usr/local/bin/nemoclawd-swarm-bot
RUN chmod +x /usr/local/bin/nemoclawd-websocket-server
RUN chmod +x /usr/local/bin/nemoclawd-solana-bridge
RUN chmod +x /usr/local/bin/nemoclawd-solana-stack
...
# Write nemoclawd.json: set nvidia as default provider, route through
...
path = os.path.expanduser('~/.nemoclawd/nemoclawd.json'); \
